              ...
              {categoryData?.map((category, index) => (
                <li key={index}>
                  <div className="category-list">
                    <Avatar data={category?.category_icon} placeHolder={categoryImage} name={category?.name} />
                    <h5>
            ...
            <ul className="nav nav-pills mb-3 custom-nav-tab">
              {categoryData?.map((category, index) => (
                <li key={index} className="nav-item">
                  <Btn className={selectedCategorySlug.length && selectedCategorySlug.includes(!category.slug) ? "nav-link active" : "nav-link"} onClick={redirectToCollection(!category.slug)}>
                    {" "}
                    {category.name}
          ...
          <Slider {...classes?.sliderOption}>
            {categoryData?.map((category, index) => (
              <div key={index}>
                {theme == "'paris'" && (
                  <div>
                    <Link href={`/category/${category?.slug}`} className={`category-box ${classes?.link} category-dark`}>
                      <div>
                        <Avatar data={category?.category_icon} placeHolder={placeHolderImage} name={category.name} />
                        <h5>{category.name}</h5>
                      </div>
          ...
          {categoryData?.map((category, index) => (
            <div key={index}>
              <div className="shop-category-box border-0" key={category.id}>
                <a className="circle-1" onClick={redirectToCollection(!category.slug)}>
      ...
      {categoryData?.length && style == "filter" && (
        <ul className="nav nav-tabs tab-style-color-2 tab-style-color">
          <li className="nav-item">
            <Btn className={!selectedCategoryId ? "nav-link btn active" : "nav-link btn"} onClick={selectCategory()}>
              {t("all")}
            </Btn>
          </li>
          {categoryData?.map((category, index) => (
            <li key={index} className="nav-item">
              <Btn className={selectedCategoryId == category.id ? "nav-link btn active" : "nav-link btn"} onClick={selectCategory(category.id)}>
                {" "}
                {category.name}
              </Btn>
            </li>
        ...
        <ul className="category-list">
          {categoryData?.map((category, index) => (
            <li key={index} className="onhover-category-list">
              <Avatar data={category?.category_icon ? category?.category_icon?.original_url : ""} name={category?.name} />
              <h6>{category?.name}</h6>
            </li>
          ))}
        </ul>
      ...
      {theme == "'cairo'" && (
        <Slider {...sliderOptions} className={classes.sliderClass}>
          {categoryData?.map((data, index) => (
            <div key={index}>
              <Link href={`/category/${data?.slug}`} className="cate-box">
                <span>{data?.name}</span>
              </Link>
            </div>
          ))}
