...
const ParisTheme = () => {
  ...
  const { data, isLoading, refetch } = useQuery(["paris"], () => request({ url: HomePageAPI, params: { slug: "paris" } }, router), { enabled: true, refetchOnWindowFocus: false, select: (res) => res?.data });
  useEffect(() => {
    isLoading && refetch();
  }, [isLoading]);
