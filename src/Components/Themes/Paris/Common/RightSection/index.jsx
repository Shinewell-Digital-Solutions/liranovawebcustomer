...
const RightSection = ({ dataAPI }) => {
  ...
  return (
    ...
    <Col xxl={dataAPI?.main_content?.sidebar?.status ? 9 : 12} xl={dataAPI?.main_content?.sidebar?.status ? 8 : 12}>
      ...
      {dataAPI?.main_content?.section3_two_column_banners?.status && (
        <div className="section-b-space section-t-space">
          <Row className="g-md-4 g-3 ratio_30">
            <Col md={6}>
              {/* imgUrl={`${ImagePath}/`} */}
              <ImageLink classes={{ customHoverClass: "banner-contain b-left" }} imgUrl={dataAPI?.main_content?.section3_two_column_banners?.banner_1?.image_url} height={156} width={579} ratioImage={false} />
            </Col>
            <Col md={6}>
              <ImageLink classes={{ customHoverClass: "banner-contain b-left" }} imgUrl={dataAPI?.main_content?.section3_two_column_banners?.banner_2?.image_url} ratioImage={false} link={dataAPI?.main_content?.section3_two_column_banners?.banner_2} height={156} width={579} />
            </Col>
          </Row>
        </div>
      )}
        ...
        <div className="section-b-space">
          <Row className="g-md-4 g-3">
            <Col xxl={8} xl={12} md={7}>
              <ImageLink classes={{ customHoverClass: "banner-contain b-left", customClass: "h-100" }} imgUrl={dataAPI?.main_content?.section6_two_column_banners?.banner_1?.image_url} ratioImage={false} link={dataAPI?.main_content?.section6_two_column_banners?.banner_1} height={283} width={781} />
            </Col>
      ...
      {dataAPI?.main_content?.section8_full_width_banner?.status && <ImageLink classes={{ customHoverClass: "banner-contain b-left", customClass: "section-b-space" }} imgUrl={dataAPI?.main_content?.section8_full_width_banner?.image_url} ratioImage={false} link={dataAPI?.main_content?.section8_full_width_banner?.redirect_link?.link_type} height={293} width={1183} />}
      ...
      {dataAPI?.main_content?.section9_featured_blogs?.status && (
        <>
          ...
          {dataAPI?.main_content?.section9_featured_blogs?.blog_ids?.length > 0 ? <BlogData dataAPI={dataAPI?.main_content?.section9_featured_blogs} svgUrl={<LeafSVG className="icon-width" />} classes={{ sliderClass: "slider-3-blog ratio_65 no-arrow product-wrapper", sliderOption: BlogOptions }} /> : <NoDataFound data={{ customClass: "bg-light no-data-added", title: "no_blog_found" }} />}
        </>
      )}
    </Col>


    {/* liranova css right section */}
    <style>
      {`
          .col-xl-8 {
        flex: 0 0 auto;
