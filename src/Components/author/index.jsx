...
const AuthorMainPage = ({ slug }) => {
    ...
    const { data: author, isLoading, refetch } = useQuery([AuthorAPI], () => request({ url: `${AuthorAPI}/slug/${slug}` }, router), { enabled: false, refetchOnWindowFocus: false, select: (res) => res?.data });
    useEffect(() => {
        if (isLoading) {
            refetch();
        }
                                    ...
                                    <div>
                                        <ul className="country-list">
                                            {author?.birth_date && <li>
                                                <span>{t("born_date")} : </span> {dateFormate(author?.birth_date)}
                                            </li>}
                                            {author?.death_date && <li>
                                                <span>{t("death_date")} : </span> {dateFormate(author?.death_date)}
                                            </li>}
                                        </ul>
                                    </div>
                                </div>
